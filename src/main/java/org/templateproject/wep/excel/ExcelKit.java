...
public class ExcelKit {
	 ...
	 * @throws IOException
	 */
	public static List<Row> readExcel(String fromExcelPath) throws IOException {

		// 扩展名为空时，
		if (fromExcelPath.equals("")) {
		...
		// 获取扩展名
		String ext = fromExcelPath.substring(fromExcelPath.lastIndexOf(".") + 1);

		try {

			if ("xls".equals(ext)) { // 使用xls方式读取
				return readExcelByXls(fromExcelPath);
			} else if ("xls".equals(ext)) { // 使用xlsx方式读取
				return readExcelByXlsx(fromExcelPath);
	 ...
	 * @throws IOException
	 */
	public void writeExcel(List<Row> rowList, String fromExcelPath, String toExcelPath) throws IOException {

		// 扩展名为空时，
		if (toExcelPath.equals("")) {
			throw new IOException("文件路径不能为空！");
		}

		// 获取扩展名
		String ext = toExcelPath.substring(toExcelPath.lastIndexOf(".") + 1);

		try {

			if ("xls".equals(ext)) { // 使用xls方式写入
				writeExcelByXls(rowList, fromExcelPath, toExcelPath);
			} else if ("xls".equals(ext)) { // 使用xlsx方式写入
				writeExcelByXlsx(rowList, fromExcelPath, toExcelPath);
	 ...
	 * @throws IOException
	 */
	public void writeExcelByXls(List<Row> rowList, String fromExcelPath, String toExcelPath) throws IOException {

		// 判断文件路径是否为空
		if (toExcelPath == null || toExcelPath.equals("")) {
			out("文件路径不能为空");
			throw new IOException("文件路径不能为空");
		}
		// 判断文件路径是否为空
		if (fromExcelPath == null || fromExcelPath.equals("")) {
			out("文件路径不能为空");
			throw new IOException("文件路径不能为空");
		}
		...
		try {
			...
			if (file.exists()) {
				...
				if (isOverWrite) {
					...
					wb = new HSSFWorkbook(new FileInputStream(fromExcelPath));
				} else {
					// 如果文件存在，则读取Excel
			...
			// 将rowlist的内容写到Excel中
			writeExcel(wb, rowList, toExcelPath);
		...
		try {
			...
			if (file.exists()) {
				...
				if (isOverWrite) {
					...
					// wb = new XSSFWorkbook();
					// wb.createSheet("Sheet1");
					wb = new XSSFWorkbook(new FileInputStream(fromExcelPath));
				} else {
					// 如果文件存在，则读取Excel
	 ...
	 * @throws Exception
	 */
	public static List<Row> readExcelByXlsx(String excelPath) throws IOException {
		// 判断文件是否存在
	 ...
	 */
	public static List<Row> readExcelByXls(String excelPath) throws IOException {
	 ...
	 * @return
	 */
	private static List<Row> readExcelToListRow(Workbook wb) {
		List<Row> rowList = new ArrayList<Row>();
